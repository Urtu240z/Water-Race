#!/usr/bin/env python3
"""Build rider_01 on rider_bot's exact canonical rest skeleton.
...
def map_mesh_to_target_rest(
    ...
    if set(source_bones) != set(target_bones):
        raise RuntimeError(
            "rider_01 and rider_bot bone sets differ: "
            f"rider_01-only={sorted(set(source_bones) - set(target_bones))}; "
...
def purge_to_rider_01(
    ...
    collection = bpy.data.collections.get("rider_01_RiderCompatible")
    if collection is None:
        collection = bpy.data.collections.new("rider_01_RiderCompatible")
...
def validate_clean_glb(
    ...
    if len(compatible_meshes) != 1:
        raise RuntimeError(
            f"Clean rider_01 GLB has {len(compatible_meshes)} skinned meshes"
        )
    importer_helpers = [
        obj.name
    ...
    if extra_objects:
        raise RuntimeError(
            f"Clean rider_01 GLB still contains helper objects: {extra_objects}"
        )
    ...
    if (
        ...
        raise RuntimeError(
            f"Clean rider_01 GLB has invalid weights: {compatible_stats}"
        )
    rest_error = shared.rest_deformation_error(compatible_meshes[0])
    if rest_error > 0.00001:
        raise RuntimeError(
            f"Clean rider_01 rest deformation is {rest_error:.9f}"
        )
    compatible_names = {
...
def main() -> None:
    ...
    if len(meshes) != 1:
        raise RuntimeError(
            f"rider_01 source has {len(meshes)} skinned meshes; expected 1"
        )
    mesh = meshes[0]
    ...
    if (
        ...
        raise RuntimeError(f"rider_01 source weights are invalid: {source_stats}")
    ...
    if maximum_influences > 4:
        raise RuntimeError(
            f"rider_01 has {maximum_influences} influences; maximum is 4"
        )
    if shared.weight_snapshot(mesh) != original_weights:
        raise RuntimeError("rider_01 numeric weights changed during mapping")
    if shared.material_names(mesh) != original_materials:
        raise RuntimeError("rider_01 material slots changed during mapping")
    if [layer.name for layer in mesh.data.uv_layers] != original_uvs:
        raise RuntimeError("rider_01 UV layers changed during mapping")
    ...
    if rest_error > 0.00001:
        raise RuntimeError(
            f"Mapped rider_01 rest deformation is {rest_error:.9f}"
        )
    print(
        "RIDER_01_PREPOSE="
        + json.dumps(
    ...
    target_armature.data.pose_position = "REST"
    mesh.name = "rider_01_Body"
    mesh.data.name = "rider_01_Body_Data"
    final_collection = purge_to_rider_01(target_armature, [mesh])
    ...
    result = {
        "title": "rider_01 RiderRig-compatible build",
        "status": "PASS",
        ...
        "final_objects": ["rider_01_Body", "SKEL_Rider"],
