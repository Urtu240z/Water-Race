#!/usr/bin/env python3
"""Build a rider_04 skin that uses rider_bot's literal armature.
...
def find_source_character() -> tuple[bpy.types.Object, bpy.types.Object]:
    ...
    if len(armatures) != 1 or len(candidates) != 1:
        raise RuntimeError(
            "rider_04 source must contain exactly one armature and one skinned mesh; "
...
def ensure_all_weighted(mesh: bpy.types.Object) -> None:
    ...
    if unweighted:
        raise RuntimeError(
            f"{len(unweighted)} rider_04 vertices have no positive influence; "
...
def similarity_fit(
    ...
    if denominator <= 1.0e-12:
        raise RuntimeError("Critical rider_04 joints are degenerate.")
...
def purge_non_final_objects(
    rider_04_mesh: bpy.types.Object,
    target_armature: bpy.types.Object,
) -> None:
    for obj in list(bpy.data.objects):
        if obj not in {rider_04_mesh, target_armature}:
...
def build(args: argparse.Namespace) -> None:
    ...
    rider_04_armature, rider_04_mesh = find_source_character()
    rider_04_armature.data.pose_position = "REST"
    source_bones = semantic_map(bone.name for bone in rider_04_armature.data.bones)
    source_groups = semantic_map(group.name for group in rider_04_mesh.vertex_groups)
    if len(source_bones) != 52 or len(source_groups) != 52:
        raise RuntimeError(
            f"Expected rider_04 52 bones/groups, found {len(source_bones)}/{len(source_groups)}."
        )
    ensure_all_weighted(rider_04_mesh)
    weights_before = weighted_snapshot(rider_04_mesh)
    ...
    if set(source_groups) != set(target_bones):
        raise RuntimeError(
            "rider_04 groups and rider_bot bones differ semantically. "
            f"rider_04-only={sorted(set(source_groups) - set(target_bones))}; "
            f"Bot-only={sorted(set(target_bones) - set(source_groups))}"
        )
    if set(source_bones) != set(target_bones):
        raise RuntimeError("rider_04 and rider_bot bone sets differ semantically.")
    hierarchy_mismatches = []
    for key in sorted(source_bones):
        source_bone = rider_04_armature.data.bones[source_bones[key]]
    ...
    alignment, scale, rotation, translation, errors = similarity_fit(
        rider_04_armature, target_armature, source_bones, target_bones
    )
    ...
    initial_lines = [
        "=== RIDER_04 RIDER-COMPATIBLE BUILD ===",
        f"Blender: {bpy.app.version_string}",
        f"Source blend: {source_blend}",
        f"Reference GLB: {reference_glb}",
        f"rider_04 bone height before fit: {character_height_from_bones(rider_04_armature):.9f} m",
    ...
    rider_04_mesh.matrix_world = alignment @ rider_04_mesh.matrix_world
    bpy.context.view_layer.update()
    for key in sorted(source_groups):
        rider_04_mesh.vertex_groups[source_groups[key]].name = target_bones[key]
    if len({group.name for group in rider_04_mesh.vertex_groups}) != 52:
        raise RuntimeError("Vertex-group rename produced a collision.")
    if weighted_snapshot(rider_04_mesh) != weights_before:
        raise RuntimeError("Numeric weights changed while renaming groups.")
    ensure_all_weighted(rider_04_mesh)

    for modifier in list(rider_04_mesh.modifiers):
        if modifier.type == "ARMATURE":
            rider_04_mesh.modifiers.remove(modifier)
    modifier = rider_04_mesh.modifiers.new(name="rider_bot_Armature", type="ARMATURE")
    modifier.object = target_armature
    mesh_world = rider_04_mesh.matrix_world.copy()
    rider_04_mesh.parent = target_armature
    rider_04_mesh.matrix_world = mesh_world
    bpy.context.view_layer.update()

    tests = deformation_tests(target_armature, rider_04_mesh, target_bones)
    action_count, nla_count = clear_animation_data()
    target_armature.name = "SKEL_Rider"
    target_armature.data.name = "SKEL_Rider"
    rider_04_mesh.name = "rider_04SkinMesh"
    rider_04_mesh.data.name = "rider_04SkinMesh_Data"
    purge_non_final_objects(rider_04_mesh, target_armature)
    images = pack_images()
    target_armature.data.pose_position = "REST"
    bpy.context.view_layer.update()
    ...
    report_write(
        ...
        [
            ...
            f"Final mesh: {rider_04_mesh.name}",
            f"Final bones/groups: {len(target_armature.data.bones)}/{len(rider_04_mesh.vertex_groups)}",
            f"All {len(rider_04_mesh.data.vertices)} vertices remain weighted: yes",
            f"Materials: {[slot.material.name for slot in rider_04_mesh.material_slots if slot.material]}",
            f"Images: {images}",
            "Deformation tests:",
