...
def rename_character(armature: bpy.types.Object, meshes: list[bpy.types.Object]) -> None:
    armature.name = "SKEL_rider_04"
    armature.data.name = "SKEL_rider_04"
    for index, mesh in enumerate(meshes, start=1):
        mesh.name = "rider_04_Mesh" if len(meshes) == 1 else f"rider_04_Mesh_{index:02d}"
...
def compare(args: argparse.Namespace) -> None:
    rider_04_path = str(Path(args.input_glb).resolve())
    reference_path = str(Path(args.reference_glb).resolve())
    rider_04 = comparison_snapshot(rider_04_path, "rider_04")
    reference = comparison_snapshot(reference_path, "rider_bot")
    common = sorted(rider_04["bones"] & reference["bones"])
    rider_04_only = sorted(rider_04["bones"] - reference["bones"])
    reference_only = sorted(reference["bones"] - rider_04["bones"])
    hierarchy_differences = [
        (
            name,
            rider_04["parents"].get(name),
            reference["parents"].get(name),
        )
        for name in common
        if rider_04["parents"].get(name) != reference["parents"].get(name)
    ]
    common_semantic = sorted(
        set(rider_04["semantic_to_actual"]) & set(reference["semantic_to_actual"])
    )
    rider_04_only_semantic = sorted(
        set(rider_04["semantic_to_actual"]) - set(reference["semantic_to_actual"])
    )
    reference_only_semantic = sorted(
        set(reference["semantic_to_actual"]) - set(rider_04["semantic_to_actual"])
    )
    semantic_hierarchy_differences = [
        (
            name,
            rider_04["semantic_parents"].get(name),
    ...
    lines = [
        "=== RIDER_04 / RIDER_BOT COMPARISON ===",
        f"rider_04: {rider_04_path}",
        f"rider_bot: {reference_path}",
        f"rider_04 height: {rider_04['height']:.6f} m",
        f"rider_bot height: {reference['height']:.6f} m",
        f"rider_04 bones: {len(rider_04['bones'])}",
        f"rider_bot bones: {len(reference['bones'])}",
        f"Common bones ({len(common)}): {common}",
        f"rider_04-only bones ({len(rider_04_only)}): {rider_04_only}",
        ...
        (
            f"Semantic rider_04-only bones ({len(rider_04_only_semantic)}): "
            f"{rider_04_only_semantic}"
        ),
        (
        ...
        f"rider_04 namespace map: {rider_04['semantic_to_actual']}",
        f"rider_bot namespace map: {reference['semantic_to_actual']}",
        f"rider_04 limb bone lengths: {rider_04['lengths']}",
        f"rider_bot limb bone lengths: {reference['lengths']}",
        f"rider_04 selected rest matrices: {rider_04['rest']}",
        f"rider_bot selected rest matrices: {reference['rest']}",
    ]
    replace_report_section(args.report, "=== RIDER_04 / RIDER_BOT COMPARISON ===", lines)
