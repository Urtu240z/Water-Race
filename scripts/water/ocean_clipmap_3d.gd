...
@export var physical_water: WaterBody3D
@export var legacy_visual_water_material: ShaderMaterial:
	set(value):
		if legacy_visual_water_material == value:
			return
		legacy_visual_water_material = value
...
var _registered_physical_water: WaterBody3D
...
func _exit_tree() -> void:
	if is_instance_valid(_registered_physical_water) and _visual_material != null:
		_registered_physical_water.unregister_external_water_material(_visual_material)
	_registered_physical_water = null
	if _visual_material != null:
		_visual_material.set_shader_parameter(&"ocean_clipmap_enabled", false)
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if physical_water == null:
		warnings.append("Assign the WaterBody3D used as the physical water provider.")
...
func _rebuild_meshes() -> void:
	...
	if _estimate_vertex_count() > MAX_SAFE_ESTIMATED_VERTICES:
		push_error(
			"OceanClipmap3D: refusing to build an unsafe mesh estimate above %d vertices."
...
func _bind_visual_material() -> void:
	...
	if candidate == null:
		if not Engine.is_editor_hint() and not _warned_missing_material:
			push_warning(
				"OceanClipmap3D: waiting for its persistent visual ShaderMaterial."
			)
	...
	if _registered_physical_water != physical_water:
		if is_instance_valid(_registered_physical_water) and _visual_material != null:
			_registered_physical_water.unregister_external_water_material(_visual_material)
		_registered_physical_water = physical_water
	if is_instance_valid(_registered_physical_water):
		_registered_physical_water.register_external_water_material(_visual_material)
	_update_material_uniforms()


func _resolve_visual_material() -> ShaderMaterial:
	return legacy_visual_water_material
...
func _update_following(force_update: bool) -> void:
	...
	if is_instance_valid(physical_water):
		resolved_water_level = physical_water.base_height
	...
	if is_instance_valid(physical_water):
		logical_origin_offset_xz = physical_water.get_logical_origin_offset_xz()
	if _debug_root != null:
...
func _update_material_uniforms() -> void:
	...
	if is_instance_valid(physical_water):
		resolved_water_level = physical_water.base_height
	_visual_material.set_shader_parameter(&"base_height", resolved_water_level)
	if is_instance_valid(physical_water):
		_visual_material.set_shader_parameter(
			&"simulation_time",
			physical_water.get_simulation_time()
		)
	_visual_material.set_shader_parameter(&"ocean_clipmap_enabled", true)
	_visual_material.set_shader_parameter(
