@tool
class_name PixelOceanWater3D
extends WaterBody3D

## Texture-driven ocean provider inspired by Taillight Games' pixel water.
##
## The same two repeating height textures are sampled by the shader and by
## this CPU provider.  Jet-ski buoyancy, effects, underwater detection and the
## rendered vertices therefore observe one surface. Dynamic ripple descriptors
## are also shared with the GPU and contribute to physical height and normals.

const MAX_RIPPLES: int = 12
const MIN_SAMPLE_STEP: float = 0.05

@export_group("Pixel Ocean")
@export var pixel_ocean_material: ShaderMaterial
...
@export var ripple_emitter_target: Node3D
...
var _pixel_simulation_time: float = 0.0
...
func _physics_process(delta: float) -> void:
	...
	_pixel_simulation_time += safe_delta
	_expire_ripples()
...
func add_ripple(
	...
	_ripple_start_times[slot] = _pixel_simulation_time
	_ripple_amplitudes[slot] = (
		wake_ripple_amplitude if amplitude < 0.0 else maxf(amplitude, 0.0)
	)
	_ripple_speeds[slot] = ripple_speed if speed < 0.0 else maxf(speed, 0.01)
	_ripple_wavelengths[slot] = (
		ripple_wavelength if wavelength < 0.0 else maxf(wavelength, 0.05)
	)
	_ripple_decays[slot] = ripple_decay if decay < 0.0 else maxf(decay, 0.0)
	_ripple_lifetimes[slot] = (
		ripple_lifetime if lifetime < 0.0 else maxf(lifetime, 0.05)
	)
...
func sample_height(world_position: Vector3) -> float:
	return base_height + _sample_surface_offset(
		world_to_logical_xz(world_position),
		_pixel_simulation_time
	)
...
func sample_normal(world_position: Vector3) -> Vector3:
	...
	var left := _sample_surface_offset(
		logical_xz - Vector2(step, 0.0),
		_pixel_simulation_time
	)
	var right := _sample_surface_offset(
		logical_xz + Vector2(step, 0.0),
		_pixel_simulation_time
	)
	var back := _sample_surface_offset(
		logical_xz - Vector2(0.0, step),
		_pixel_simulation_time
	)
	var front := _sample_surface_offset(
		logical_xz + Vector2(0.0, step),
		_pixel_simulation_time
	)
...
func sample_water_velocity(world_position: Vector3) -> Vector3:
	...
	var previous_height := _sample_surface_offset(
		logical_xz,
		_pixel_simulation_time - TIME_STEP
	)
	var next_height := _sample_surface_offset(
		logical_xz,
		_pixel_simulation_time + TIME_STEP
	)
	return Vector3(
		0.0,
		(next_height - previous_height) / (2.0 * TIME_STEP),
		0.0
	)
...
func sample_crest_metric(world_position: Vector3) -> float:
	var height_offset := sample_height(world_position) - base_height
...
func _sample_macro_height(logical_xz: Vector2, sample_time: float) -> float:
	...
	var uv_a := (
		...
		+ direction_a * sample_time * wave_travel_speed_a
			/ maxf(wave_world_size_a, 0.001)
	)
	var uv_b := (
		logical_xz / maxf(wave_world_size_b, 0.001)
		+ direction_b * sample_time * wave_travel_speed_b
			/ maxf(wave_world_size_b, 0.001)
		+ Vector2(0.307, 0.476)
	)
	var height_a := (
		_sample_image_repeat(_wave_image_a, uv_a) - wave_mean_a
	) * wave_amplitude_a
	var height_b := (
		_sample_image_repeat(_wave_image_b, uv_b) - wave_mean_b
	) * wave_amplitude_b
...
func _sample_ripple_height(logical_xz: Vector2, sample_time: float) -> float:
	...
	for index in MAX_RIPPLES:
		...
		height += (
			sin(shell_offset * TAU / wavelength)
			* _ripple_amplitudes[index]
			* envelope
		)
	return height
...
func _sample_image_repeat(image: Image, uv: Vector2) -> float:
	...
	var top := lerpf(
		image.get_pixel(x_0, y_0).r,
		image.get_pixel(x_1, y_0).r,
		weight_x
	)
	var bottom := lerpf(
		image.get_pixel(x_0, y_1).r,
		image.get_pixel(x_1, y_1).r,
		weight_x
	)
...
func _expire_ripples() -> void:
	for index in MAX_RIPPLES:
		if (
			_ripple_active[index] != 0
			and _pixel_simulation_time - _ripple_start_times[index]
				> _ripple_lifetimes[index]
		):
			_ripple_active[index] = 0
...
func _update_automatic_wake(delta: float) -> void:
	...
	if (
		not close_to_surface
		or speed < wake_minimum_speed
		or travelled < wake_ripple_spacing
	):
	...
	add_ripple(
		wake_position,
		wake_ripple_amplitude * lerpf(0.55, 1.25, speed_factor)
	)
	_last_emitter_logical_xz = logical_xz
...
func _build_runtime_height_maps_when_ready() -> void:
	...
	for _attempt in 60:
		...
		if _build_runtime_height_maps():
			...
			return
	push_warning(
		"PixelOceanWater3D could not build its synchronized height maps; "
		+ "physical sampling will remain flat."
	)


func _build_runtime_height_maps() -> bool:
	# NoiseTexture2D is generated asynchronously and can expose a uniform
	# placeholder to CPU callers. Generate the seamless images synchronously,
	# then give those exact ImageTextures to both physics and every shader.
		...
		for pixel_y in height:
			for pixel_x in width:
				...
				image.set_pixel(
					pixel_x,
					pixel_y,
					noise_texture.color_ramp.sample(noise_value)
				)
	return image
...
func _push_all_shader_parameters() -> void:
	for material in _all_water_materials():
		_push_common_shader_parameters(material)


func _push_common_shader_parameters(material: ShaderMaterial) -> void:
	if material == null or material.shader == null:
		return
	material.set_shader_parameter(&"pixel_ocean_enabled", true)
	material.set_shader_parameter(&"base_height", base_height)
	material.set_shader_parameter(&"simulation_time", _pixel_simulation_time)
	...
	material.set_shader_parameter(
		...
		_runtime_wave_texture_a
		if _runtime_wave_texture_a != null
		else wave_height_texture_a
	)
	material.set_shader_parameter(
		&"wave_height_texture_b",
		_runtime_wave_texture_b
		if _runtime_wave_texture_b != null
		else wave_height_texture_b
	)
	...
	material.set_shader_parameter(
		&"wave_direction_a",
		_safe_direction(wave_direction_a, Vector2.RIGHT)
	)
	material.set_shader_parameter(
		&"wave_direction_b",
		_safe_direction(wave_direction_b, Vector2.DOWN)
	)
...
func _all_water_materials() -> Array[ShaderMaterial]:
	var materials: Array[ShaderMaterial] = []
	if pixel_ocean_material != null and pixel_ocean_material.shader != null:
		materials.append(pixel_ocean_material)
