...
@export_range(2.0, 300.0, 0.5, "or_greater", "suffix:m") var wake_length: float = 160.0
...
@export var legacy_global_deformation_enabled: bool = true
...
func _configure_wake_trail() -> void:
	...
	# The ribbon only carries foam. Real crests/depressions are generated by
	# Ocean3D; keeping this lift small prevents the foam from becoming a wall.
	_wake_trail.local_visual_crest_height = 0.10
	_wake_trail.local_visual_center_depression = 0.025
	_wake_trail.local_visual_center_turbulence_height = 0.0
	...
	# Distance normally triggers first. This interval prevents a stationary or
	# slow source from filling the 128-point history before it reaches 20 s.
	_wake_trail.wake_sample_maximum_interval = 0.25
	var effective_lifetime := wake_fade
	var sample_distance := clampf(wake_length / 32.0, 0.5, 1.5)
	var maximum_points := clampi(ceili(wake_length / sample_distance) + 2, 12, 128)
...
func _update_wakes() -> void:
	...
	_update_physical_wake(
		current_speed
		if (
			_camera_effects_active
			and legacy_global_deformation_enabled
			and not navigable_wake_enabled
		)
		else 0.0
	)
