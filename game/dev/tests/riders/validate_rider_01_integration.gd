...
const MESH_PATH := "res://gameplay/riders/rider_01/runtime/rider_01_body_mesh.res"
const SKIN_PATH := "res://gameplay/riders/rider_01/runtime/rider_01_skin.res"
const GROUP := &"rider_skin_rider_01"
const SKIN_VALUE := 3
const REPORT_PATH := "user://rider_01_integration_report.txt"
...
func _run() -> void:
	_report.append("=== RIDER_01 INTEGRATION VALIDATION ===")
	_validate_resources()
	await _validate_rig()
...
func _validate_resources() -> void:
	...
	_expect(mesh != null, "rider_01 persistent ArrayMesh loads.")
	_expect(skin != null, "rider_01 persistent Skin loads.")
	if mesh != null:
		_expect(mesh.get_surface_count() == 1, "rider_01 keeps one mesh surface.")
		_expect(_vertex_count(mesh) == 27992, "rider_01 keeps 27,992 vertices.")
	if skin != null:
		_expect(skin.get_bind_count() == 52, "rider_01 keeps 52 skin binds.")
...
func _validate_rig() -> void:
	...
	_expect(rider_meshes.size() == 1, "RiderRig has one rider_01 body mesh.")
	if skeletons.size() == 1 and rider_meshes.size() == 1:
		...
		_expect(mesh.skin != null and mesh.skin.get_bind_count() == 52, "rider_01 resolves 52 binds.")
		_expect(mesh.get_node_or_null(mesh.skeleton) == skeleton, "rider_01 targets the canonical skeleton.")
	rig.call("set_rider_skin", SKIN_VALUE)
	await process_frame
	_expect(_visible_count(rider_meshes) == 1, "Serialized value 3 selects physical rider_01.")
	rig.call("set_rider_skin", 0)
	await process_frame
	_expect(_visible_count(rider_meshes) == 0, "rider_bot selection hides rider_01.")
	rig.queue_free()
	await process_frame
